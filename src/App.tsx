...
export default function App() {
  ...
  const changeLanguage = (nextLanguage: LocaleCode) => {
    ...
    routerNavigate(
      `${location.pathname}?lang=${encodeURIComponent(nextLanguage)}`,
    )
  }
  ...
  return (
    <ErrorBoundary
      title={lang === "km" ? "មានបញ្ហាកើតឡើង" : lang === "zh-CN" ? "页面出现问题" : "Something went wrong"}
      message={lang === "km" ? "យើងមិនអាចបង្ហាញទំព័រនេះបានទេ។ សូមព្យាយាមម្តងទៀត។" : lang === "zh-CN" ? "我们暂时无法显示此页面，请重试。" : "We could not display this page. Please try again."}
      retryLabel={lang === "km" ? "ព្យាយាមម្តងទៀត" : lang === "zh-CN" ? "重试" : "Try again"}
    >
      <div
      ...
      <Header
        ...
        onSearch={goSearch}
      />
      {!isOnline && <OfflineBanner message={lang === "km" ? "អ្នកកំពុងក្រៅបណ្តាញ។ មុខងារមួយចំនួនអាចមិនអាចប្រើបាន។" : lang === "zh-CN" ? "您当前处于离线状态，部分功能可能无法使用。" : "You are offline. Some features may be unavailable."} />}
      ...
      <main style={{ flex: 1 }} id="main-content">
        ...
        {page === "promotions" && <PromotionsPage navigate={navigate} />}
        ...
        {page === "announcements" && <AnnouncementsPage navigate={navigate} />}
